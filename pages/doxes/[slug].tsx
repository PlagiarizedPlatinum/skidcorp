...
export default function StoryPage({ story }: { story: Story }) {
  if (!story) return (
    <div className="page-center">
      ...
      <Link href="/doxes" className="btn" style={{ marginTop: '2rem' }}>← All doxes</Link>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: '760px', width: '90%', margin: '0 auto', padding: '4rem 0 6rem' }}>
      <div style={{ marginBottom: '3rem', display: 'flex', gap: '1rem' }}>
        <Link href="/doxes" className="btn" style={{ fontSize: '0.75rem' }}>← All doxes</Link>
...
export const getStaticPaths: GetStaticPaths = async () => {
  try {
    const sql = getDb();
    const doxes = await sql`SELECT slug FROM doxes`;
    return {
      paths: doxes.map(s => ({ params: { slug: s.slug } })),
      fallback: 'blocking',
    };
  } catch {
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  try {
    const sql = getDb();
    const [story] = await sql`SELECT * FROM doxes WHERE slug = ${params!.slug as string}`;
