...
export default function doxPage({ dox }: { dox: dox }) {
  if (!dox) return (
    ...
    <div style={{ maxWidth: '760px', width: '90%', margin: '0 auto', padding: '4rem 0 6rem' }}>
      ...
      <div style={{ marginBottom: '2.5rem' }}>
        <p style={{ fontSize: '0.7rem', letterSpacing: '3px', textTransform: 'uppercase', color: 'var(--dim)', marginBottom: '0.8rem' }}>
          {new Date(dox.created_at).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}
        </p>
        <h1 style={{
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  try {
    const sql = getDb();
    const [dox] = await sql`SELECT * FROM dox WHERE slug = ${params!.slug as string}`;
    if (!dox) return { notFound: true };
    return {
      props: { dox: { ...dox, created_at: dox.created_at.toISOString() } },
      revalidate: 60,
    };
  } catch {
