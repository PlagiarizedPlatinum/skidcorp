  ...
  if (!story) {
    return (
      <div className="page-center">
        ...
        <Link href="/dox" className="btn" style={{ marginTop: '2rem' }}>
          ← All Dox
        </Link>
      </div>
    );
  ...
  return (
    ...
    >
      <div style={{ marginBottom: '2.5rem', display: 'flex', gap: '1rem' }}>
        <Link href="/dox" className="btn" style={{ fontSize: '0.75rem' }}>
          ← All Dox
        </Link>
        <Link href="/" className="btn" style={{ fontSize: '0.75rem' }}>
          Home
        </Link>
      </div>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  try {
    ...
    return {
      props: {
        story: { ...story, created_at: story.created_at.toISOString() },
      },
      revalidate: 60,
    };
  } catch {
